...
h3=subplot(2,3,3);
plot(f,abs(w1)/1000,f,abs(w2)/1000,f,abs(w3)/1000,f,abs(w4)/1000,f,abs(w5)/1000,f,abs(w6)/1000,f,abs(w7)/1000,f,abs(w8)/1000);grid on
...
for ii=0:(length(tone_all_2)/1421-1)
    ...
    %% Using modified Goertzel Algorithm to find X(k) at our desired freq. bins
    ...
    m=2*m/205; % Using the (2/N) *|X(K)|^2 formula
    %% Setting up the threshoold for digital logic converision
    th=sum(m)/4;  
    f=[697 770 852 941 1209 1336 1477 1633];
    f1=[0  4000];
    th=[th th];
    
    %% Search for the frequency component present
    idx=find(m>th(1));
    Determination=f(idx);
    switch Determination(1)
            case {697}      % Along 1st row
